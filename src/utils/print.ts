...
const Print = function (dom: any, options?: object): PrintFunction {
  options = options || {}
  if (!(this instanceof Print))
    return Print(dom, options)
...
Print.prototype = {
  ...
  getStyle(): string {
    let str = ''
    const styles: NodeListOf<Element>
  ...
  getHtml(): Element {
    ...
    for (let k4 = 0; k4 < canvass.length; k4++) {
      ...
      canvass[k4].parentNode.insertBefore(
        img,
        canvass[k4].nextElementSibling,
      )
    }
